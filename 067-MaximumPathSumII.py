#Problem 67: By starting at the top of the triangle below and moving to adjacent numbers on the row 
...
8 5 9 3
"""
#That is, 3 + 7 + 4 + 9 = 23. Find the maximum total from top to bottom in triangle.txt (hyperlink 
#to https://projecteuler.net/resources/documents/0067_triangle.txt)(right click and 'Save 
#Link/Target As...'), a 15K text file containing a triangle with one-hundred rows. NOTE: This is a 
#much more difficult version of Problem 18. It is not possible to try every route to solve this 
#problem, as there are 299 altogether! If you could check one trillion (1012) routes every second it 
...
array=[]
row=[]
for i in range(0, len(bigString1)):
    if bigString1[i]==" ":
        row.append(int(bigString1[i-2:i]))
    if bigString1[i]=="n":
        array.append(row)
        row=[]
for i in range(0, len(bigString2)):
    if bigString2[i]==" ":
        row.append(int(bigString2[i-2:i]))
    if bigString2[i]=="n":
        array.append(row)
        row=[]
...
def value(row, col):
    if v[row][col]==-1:
        if row==99:
            ...
            if m>n:
                n=m
            v[row][col]=array[row][col]+n
            return array[row][col]+n
    else:
        return v[row][col]
print("ANSWER:", value(0, 0))
