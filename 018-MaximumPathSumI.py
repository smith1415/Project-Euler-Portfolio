#Problem 18: By starting at the top of the triangle below and moving to adjacent numbers 
...
#finding the upper bound, summing the max of each row
"""maxArray=[]
indices=[]
for i in range(0, 14):
    candidate=array[i][0]
    candidateIndex=0
    for j in range (1, i):
        if array[i][j] > candidate:
            candidate=array[i][j]
            candidateIndex=j
    maxArray.append(candidate)
...
#finding the lower bound (starting from the top and blindly choosing down)
"""sum=array[0][0]
j=0
for i in range(1,14):
